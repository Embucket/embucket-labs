...
This guide covers `snowflake-cli` configuration to connect to a local Embucket instance and execute SQL queries through the Snowflake-compatible API. This guide covers local development setup only. Production deployments, advanced authentication, and performance optimization aren't covered.
...
<Aside type="note">
  The `snowflake-cli` is the official command-line tool that provides Snowflake's SQL interface.
  Embucket exposes a Snowflake-compatible API endpoint that accepts standard Snowflake SQL commands.
</Aside>
...
2. **Locate your configuration file**
   ...
   Find your Snowflake CLI configuration file:
...
- **Connection refused or timeout errors:** Verify Embucket container is running on ports 3000 and 8080
- **Protocol errors:** Check that `protocol = "http"` is set in your configuration file
- **Authentication failures:** Use exactly `embucket` for both username and password
- **SQL execution errors:** Check that you're using Snowflake-compatible SQL syntax

## Next steps

Execute advanced workflows with `snowflake-cli` and Embucket:

- Execute complex analytical queries with familiar Snowflake SQL syntax
