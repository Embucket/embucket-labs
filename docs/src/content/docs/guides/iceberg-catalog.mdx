...
This guide shows how to connect external Iceberg clients to Embucket's Rest API and create tables using `pyiceberg`. It doesn't cover Apache Spark or Trino integrations, advanced Apache Iceberg features, or production deployment considerations.
...
<Aside type="caution">
Select your volume type based on deployment needs. Memory volumes store data in Embucket's memory, making them inaccessible to external Iceberg clients. Filesystem volumes store data on disk. They work only when clients share the same filesystem with Embucket. S3 volumes work with all deployment configurations.
</Aside>
...
Since Embucket is a Snowflake compatible database, it uses database terminology, while Iceberg uses warehouse terminology. At the same time, Embucket also provides an Iceberg Catalog API, that operate on warehouses, namespaces and tables. Following table shows the mapping between Embucket and Iceberg entities:
...
2. Load it into your PyArrow dataframe:
...
4. Append the dataframe to the table:
