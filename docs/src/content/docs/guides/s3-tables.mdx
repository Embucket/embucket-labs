...
You can also provision the same volume at startup by providing a YAML file via
`--metastore-config`:
...
Run `embucketd --metastore-config path/to/file.yaml --no-bootstrap` to load the volume before the HTTP
API starts.
