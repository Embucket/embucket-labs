...
- Validate results and explore generated analytics models
- Plan next steps for production deployment

## What this guide covers

This guide covers local development setup with sample data and basic dbt transformations. This guide doesn't cover production deployment, advanced dbt customizations, custom Snowplow tracking implementation, or enterprise security configurations.

<Aside type="note">
  For complete example, see [dbt integration tests](https://github.com/Embucket/embucket/tree/main/test/dbt_integration_tests/dbt-snowplow-web).
</Aside>
...
Add the Snowplow Web package (a dbt package for web analytics transformations) to your `packages.yml`:
...
<Aside type="tip">
**Data Persistence**: Embucket creates in-memory storage and `embucket` database by default. For persistent data, run Embucket with `--no-bootstrap` and configure external storage. See the [volumes guide](/essentials/volumes) for details.
</Aside>
...
This setup directs the Snowplow Web package to read from `analytics.snowplow.events` and process data for August 19-26, 2022.
...
2. Install the required dbt packages:

    ```bash
    pip install dbt-core dbt-snowflake
    ```

3. Download dbt dependencies:
...
Download the Snowplow sample dataset:
...
Load reference data required by the Snowplow Web package:
