...
[Apache Iceberg](https://iceberg.apache.org/) is an open table format that defines how to store and manage large datasets.
...
- Views
- Table maintenance operations (compaction, snapshot expiration)
- `UPDATE` and `DELETE` operations
- `ALTER TABLE` operations

<Aside type="tip">
You can delegate maintenance operations to external tools like AWS S3 table buckets and Apache Spark.
</Aside>
...
<Aside type="note">
Not all endpoints are implemented. Some existing endpoints aren't fully compatible with the Apache Iceberg specification.
</Aside>
