...
<Aside type="caution">
  `MERGE INTO` operations rewrite entire Parquet files, which can be expensive for large files with
  small changes.
</Aside>
...
<Aside type="note">
  Not all endpoints are implemented. Some existing endpoints aren't fully compatible with the Apache
