...
Embucket ships with an embedded metastore and keeps metadata inside the process. When you configure S3 volumes, metadata snapshots are written next to your Iceberg tables, so there are no additional services to manage.
...
| `--metastore-config` | `METASTORE_CONFIG`   | _unset_ | Path to a YAML file that seeds volumes, databases, and schemas before startup     |

<Aside type="note">
  The bootstrap process creates an in-memory volume, default database named `embucket`, and default
  schema named `public`. Provide a metastore config to create your own external volumes instead.
</Aside>

#### Metastore config files

Point `--metastore-config` at a YAML file to pre-provision the in-memory metastore:
...
Each volume entry is created if missing and (optionally) wires up a database pointing at it. You can also
list additional databases and schemas in `databases:` and `schemas:` sections for more advanced layouts.
