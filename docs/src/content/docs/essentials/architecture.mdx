...
Embucket ships with a lightweight in-memory metastore:

- **No external services**: Metadata lives inside the Embucket process
- **Object storage native**: Table metadata snapshots are written next to your Iceberg data
- **Fast startup**: State is reconstructed from object storage, so nodes remain stateless
- **Simple ops**: Nothing to provision or babysit
