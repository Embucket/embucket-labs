...
Embucket delivers a Snowflake-compatible lakehouse through a zero-disk architecture where all persistent state resides in object storage. This document explains how Embucket's components work together to provide a complete analytics platform in a single binary.
...
Embucket implements a zero-disk lakehouse architecture where nodes maintain no persistent state. Object storage serves as the single source of truth for both data and metadata. This design eliminates operational complexity while delivering horizontal scalability and fault tolerance.
...
- **Embedded Log-Structured Merge (LSM) tree**: No external database dependencies
- **Object storage native**: Writes Sorted String Table (SST) files directly to S3
...
Embucket implements Snowflake's SQL grammar and REST API:
