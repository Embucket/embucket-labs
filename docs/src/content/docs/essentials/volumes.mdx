...
Volumes are database objects that define storage locations for your data in Embucket. A volume contains configuration settings that reference object storage systems like Amazon S3, along with access credentials and backend specifications. You create databases that use these volumes to store their tables and metadata. This document covers how to configure and manage volumes in your Embucket deployment.
...
Each volume is a storage configuration object that contains:
...
S3 volumes are database objects that store data in S3-compatible object storage systems. S3-compatible systems implement the Amazon S3 API standard for data access and management. Use S3 volumes for production deployments.
...
S3 tables volumes are database objects that store data in AWS S3 table buckets. Unlike standard S3 volumes that work with any S3-compatible storage, S3 tables volumes specifically integrate with AWS S3 table buckets for optimized performance. Use S3 tables volumes for AWS-native production deployments.
...
Filesystem volumes are database objects that store data on the local file system. Unlike cloud-based volumes, filesystem volumes write data directly to disk paths on the server where Embucket runs. Use filesystem volumes only for development and testing.
...
Memory volumes are database objects that store data entirely in system memory (RAM). Unlike persistent storage volumes, memory volumes lose all data when the system restarts. Use memory volumes only for temporary testing.
