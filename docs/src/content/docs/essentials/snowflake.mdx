...
Embucket provides an open source Snowflake alternative with the same ease of use and manageability. You gain open source and self-hosting benefits. The system uses open technologies including [Apache DataFusion](https://datafusion.apache.org/), [Apache Iceberg](https://iceberg.apache.org/), [Apache Arrow](https://arrow.apache.org/), and Parquet, plus a lightweight embedded metastore.

These technology choices differ structurally:

- **Metadata storage**: Embedded metastore synchronized with object storage instead of FoundationDB distributed database
