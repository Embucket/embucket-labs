...
def rest_spark_session():
    spark_session = spark_session_factory(
        config_type="file_catalog", app_name="FileCatalogTest"
    )
    yield spark_session
    spark_session.stop()
...
def s3_spark_session():
    spark_session = spark_session_factory(
        config_type="s3_catalog", app_name="S3CatalogTest"
    )
    yield spark_session
    spark_session.stop()
...
def spark_session_factory(
    config_type: str, app_name: str, config_overrides: dict = None
):
    """
    ...
    if config_type == "file_catalog":
        builder.config(
            "spark.jars.packages",
            "org.apache.iceberg:iceberg-spark-runtime-3.5_2.12:1.9.1,org.apache.hadoop:hadoop-aws:3.3.4",
        ).config(
            ...
            "spark.sql.catalog.rest.io-impl", "org.apache.iceberg.hadoop.HadoopFileIO"
        ).config(
            "spark.sql.catalog.rest.uri", CATALOG_URL
        ).config(
            "spark.sql.catalog.rest.warehouse", WAREHOUSE_ID
        ).config(
    ...
    # --- Configuration Type 2: Direct S3 Access with Explicit Keys ---
        ...
        builder.config(
            "spark.jars.packages",
            "org.apache.iceberg:iceberg-spark-runtime-3.5_2.12:1.9.1,org.apache.hadoop:hadoop-aws:3.3.4,com.amazonaws:aws-java-sdk-bundle:1.12.262",
        ).config(
        ...
        raise ValueError(
            f"Unknown config_type: '{config_type}'. Expected 'file_catalog' or 's3_catalog'."
        )
