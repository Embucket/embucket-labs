...
class SQLLogicPythonRunner:
    ...
    def compare_with_remote_results(self):
        """Compare local test results with the version from the main branch"""
        try:
            print("\nComparing with main branch results...")

            # Create a temporary file to store the main branch version
            temp_file = "main_test_statistics.csv"

            # Use git show to get the content of the file from the main branch
            import subprocess
            result = subprocess.run(
                ["git", "show", "origin/main:test/test_statistics.csv"],
            ...
            # Load both files into dictionaries for comparison
            local_results = {}
            main_results = {}
            ...
            for key in common_tests:
                ...
                if local['success_percentage'] > main['success_percentage']:
                    ...
                    # Check if any tests that were passing in main are now failing
            ...
            # Show newly failing tests - tests that were passing in main but now failing
            if new_failures:
    ...
    def run(self):
        ...
        if benchmark_mode:
            ...
            self.run_files_parallel(config, file_paths, test_directory, benchmark_mode, default_run_mode, start_time, is_embucket,
                                    workers)
            self.compare_with_remote_results()
