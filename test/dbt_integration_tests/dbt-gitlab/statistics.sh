...
grep '^[[:space:]][[:space:]]000200:' run.log | awk -F'000200: ' '{print $2}' | sort | uniq -c | sort -nr | head -n 10 >> assets/top_errors.txt
...
grep '^[[:space:]][[:space:]]000200:.*Invalid function' run.log | awk -F'000200: ' '{print $2}' | sort | uniq -c | sort -nr | head -n 15 >> assets/top_errors.txt