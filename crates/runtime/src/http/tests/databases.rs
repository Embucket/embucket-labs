...
use icebucket_metastore::{
    IceBucketDatabase, IceBucketVolume,
};
use icebucket_metastore::{IceBucketVolumeType};
use crate::http::tests::common::{Op, Entity, create_server, ui_test_op};
...
async fn test_ui_databases_metastore_update_bug() 
{
    ...
    let res = ui_test_op(addr, Op::Create, None, &Entity::Volume(IceBucketVolume {
        ident: String::new(),
    ...
    let res = ui_test_op(addr, Op::Get, None, &Entity::Database(created_database.clone())).await;
    ...
    let res = ui_test_op(addr, Op::Get, None, &Entity::Database(renamed_database.clone())).await;
    assert_eq!(http::StatusCode::OK, res.status());
    let error = res.json::<ErrorResponse>().await.unwrap();
...
async fn test_ui_databases() {
    ...
    // Delete database, Ok
    let res = ui_test_op(addr, Op::Delete, Some(&Entity::Database(created_database)), &stub).await;
    assert_eq!(http::StatusCode::OK, res.status());
