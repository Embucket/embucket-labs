...
use axum::{
    body::Body, extract::Path, http::{header, StatusCode}, response::{IntoResponse, Redirect, Response}
};
use mime_guess;
use std::io::Cursor;
use std::io::Read;
...
use super::error::{
    ...
    BadArchiveSnafu,
    ResponseBodySnafu,
    NonUnicodeEntryPathInArchiveSnafu,
    ReadEntryDataSnafu,
};

// Alternative to using tarball is the rust-embed package

const WEB_ASSETS_TARBALL: &[u8] = include_bytes!("../../../../../dist.tar");
...
fn get_file_from_tar(file_name: &str) -> Result<Vec<u8>> {
    ...
    Err(HandlerError(
        ...
        ArchiveError::NotFound { path: file_name.to_string() },
    ))
}


pub async fn tar_handler(
