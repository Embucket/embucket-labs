...
impl ArrayCompactUDF {
    ...
    fn compact_array(&self, array_str: impl AsRef<str>) -> DFResult<String> {
        ...
        if let Value::Array(array) = array_value {
            ...
            let compacted: Vec<&Value> = array
                .iter()
                .filter(|&v| !v.is_null() && v != &Value::Null)
                .collect();

            // Create a new array with the filtered values
            let compacted_array = Value::Array(compacted.into_iter().cloned().collect());
