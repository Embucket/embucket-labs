...
pub struct HistoryStoreDb {
    pub queries_db: SqliteDb,
    pub results_db: SqliteDb,
}

impl std::fmt::Debug for HistoryStoreDb {
...
impl HistoryStoreDb {
    ...
    pub async fn new(history_db_name: String, results_db_name: String) -> Result<Self> {
        ...
        let history_store = Self {
            queries_db: SqliteDb::new(&history_db_name)
                .await
                .expect("Failed to initialize sqlite store"),
            results_db: SqliteDb::new(&results_db_name)
                .await
                .expect("Failed to initialize sqlite store"),
        };
    ...
    pub async fn new_in_memory() -> Self {
        ...
        let store = Self {
            queries_db: SqliteDb::new(&queries_db_name)
                .await
...
impl HistoryStoreDb {
    ...
    pub async fn create_tables(&self) -> Result<()> {
        let queries_connection = self
            ...
            .conn()
            .await
            .context(history_err::SqliteConnSnafu)?;
...
#[async_trait]
impl HistoryStore for HistoryStoreDb {
    #[instrument(
    ...
    async fn get_worksheet(&self, id: WorksheetId) -> Result<Worksheet> {
        ...
        if res == Err(rusqlite::Error::QueryReturnedNoRows) {
            ...
            let worksheet = res.context(history_err::WorksheetGetSnafu)?;
    ...
    async fn update_query(&self, item: &QueryRecord, result_set: Option<ResultSet>) -> Result<()> {
        ...
        if let Some((serialized_result, serialized_rows_count)) = serialized {
            ...
            let updated_queries = res.0.context(history_err::QueryUpdateSnafu)?;
            let inserted_results = res.1.context(history_err::ResultAddSnafu)?;
    ...
    async fn get_query(&self, id: QueryRecordId) -> Result<QueryRecord> {
        ...
        if res == Err(rusqlite::Error::QueryReturnedNoRows) {
            ...
            let query = res.context(history_err::QueryGetSnafu)?;
            tracing::Span::current().record("ok", true);
            Ok(query)
        }
    }
