...
pub enum AuthStoreError {
    ...
    #[snafu(display("Error adding token: {source}"))]
    TokenAdd { source: embucket_utils::Error },
...
pub trait AuthStore: std::fmt::Debug + Send + Sync {
    async fn add_token(&self, token: Token) -> AuthStoreResult<Token>;
