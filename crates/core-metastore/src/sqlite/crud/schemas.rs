...
impl TryInto<RwObject<Schema>> for (SchemaRecord, DatabaseIdent) {
    ...
    fn try_into(self) -> Result<RwObject<Schema>> {
        ...
        Ok(RwObject::new(Schema::new(SchemaIdent {
            schema: self.0.name,
        ...
        .with_updated_at(
            DateTime::parse_from_rfc3339(&self.0.updated_at)
                .context(metastore_err::TimeParseSnafu)?
                .with_timezone(&Utc),
        ))
    }
...
) -> Result<Option<RwObject<Schema>>> {
    let mut items = list_schemas(
        conn,
        ListParams::default().by_name(schema_ident.schema.clone()),
    )
    .await?;
    if items.is_empty() {
        Ok(None)
...
pub async fn list_schemas(conn: &Connection, params: ListParams) -> Result<Vec<RwObject<Schema>>> {
    conn.interact(move |conn| {
        ...
        query.load::<(SchemaRecord, String)>(conn)
    })
    .await?
    .context(metastore_err::DieselSnafu)?
    .into_iter()
...
pub async fn update_schema(
    ...
    let database = get_database(conn, &ident.database).await?.context(
