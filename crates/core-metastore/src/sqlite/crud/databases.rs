...
pub async fn get_database(
    conn: &Connection,
    database_ident: &DatabaseIdent,
) -> Result<Option<RwObject<Database>>> {
    let mut items =
        list_databases(conn, ListParams::default().by_name(database_ident.clone())).await?;
    if items.is_empty() {
        Ok(None)
    } else {
