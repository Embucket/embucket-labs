...
impl BasicMetastore {
    ...
    pub fn from_yaml_file(path: &str) -> Result<Self> {
        let config = MetastoreConfig::from_yaml_file(path)
            .map_err(|e| metastore_error::NotYetImplementedSnafu {
    ...
    pub fn from_yaml_str(yaml: &str) -> Result<Self> {
        let config = MetastoreConfig::from_yaml_str(yaml)
            .map_err(|e| metastore_error::NotYetImplementedSnafu {
...
impl Metastore for BasicMetastore {
    ...
    async fn create_volume(&self, _name: &VolumeIdent, _volume: Volume) -> Result<RwObject<Volume>> {
        Self::not_implemented("create_volume - use config to define volumes")?;
        unreachable!()
    }

    async fn update_volume(&self, _name: &VolumeIdent, _volume: Volume) -> Result<RwObject<Volume>> {
    ...
    async fn create_schema(&self, _ident: &SchemaIdent, _schema: Schema) -> Result<RwObject<Schema>> {
    ...
    async fn update_schema(&self, _ident: &SchemaIdent, _schema: Schema) -> Result<RwObject<Schema>> {
    ...
    async fn table_object_store(&self, _ident: &TableIdent) -> Result<Option<Arc<dyn ObjectStore>>> {
        Ok(None)
    }
...
"#;
        ...
        let result = metastore
            .create_volume(&VolumeIdent("test".to_string()), Volume {
