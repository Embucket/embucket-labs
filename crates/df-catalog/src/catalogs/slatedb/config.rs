...
impl SlateDBViewConfig {
    pub async fn make_volumes(
        ...
        for volume in volumes {
            builder.add_volume(&volume.ident, volume.volume.to_string(), volume.created_at.to_string(), volume.updated_at.to_string());
        }
        Ok(())
    }
    ...
    pub async fn make_databases(
        ...
        for database in databases {
            builder.add_database(database.ident.as_str(), &database.volume, database.created_at.to_string(), database.updated_at.to_string());
        }
        Ok(())
    }
